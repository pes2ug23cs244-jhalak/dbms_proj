...
DB_CONFIG = {
    ...
    'password': 'jkna0165',   # empty string if no password
...
# ------------------ HELPER FUNCTIONS ------------------
# Function to call the MySQL FUNCTION
def show_total_sold_artworks():
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("SELECT total_sold_artworks();")  # calling your MySQL function
        result = cur.fetchone()
        total = result[0] if result else 0
        messagebox.showinfo("Total Sold Artworks", f"Total sold artworks: {total}")
...
class DBMSApp(tk.Tk):
    def __init__(self):
        ...
        tables = [
            ...
            ("bid", ["buyer_ID","artwork_ID","amount","bid_date","auction_ID"], "buyer_ID"),  # composite PK simplified for CRUD
